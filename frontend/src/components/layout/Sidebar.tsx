...
export default function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={cn(
        "fixed inset-y-0 left-0 w-64 bg-slate-900 text-white z-50 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0",
        ...
        <div className="flex flex-col h-full">
          <div className="p-4 lg:p-6 border-b border-slate-700 flex items-center justify-between">
            <Link href="/dashboard" className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-sm lg:text-lg">Emergency</h1>
                <p className="text-xs text-slate-400 hidden lg:block">Alert System</p>
              </div>
            </Link>
            <button 
              onClick={onClose}
            ...
            {user?.role === 'admin' && (
              <>
                <div className="pt-4 pb-2">
                  <p className="px-3 lg:px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                    Admin
                  </p>
                </div>
